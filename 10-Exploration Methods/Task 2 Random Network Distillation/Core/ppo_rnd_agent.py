...
class Brain:
    ...
    def calculate_int_rewards(self, next_obs, batch=True):
        ...
        target_feat = self.target_model(norm_obs)
        pred_feat = self.predictor_model(norm_obs)
        return self.mse_loss(target_feat, pred_feat)
    ...
    def calculate_rnd_loss(self, obs):
        ...
        se = self.mse_loss(pred_feat, target_feat)
        mask = (torch.rand_like(se) < self.config["predictor_proportion"]).float().to(self.device)
        final_loss = se.mul(mask).sum() / mask.sum()
        return final_loss
